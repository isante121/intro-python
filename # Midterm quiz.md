...
# Write your response using complete sentences.
1.Syntax error refers to a mistake in code that violates the programming language’s rules and prevents it from working correctly.
2.A TypeError means that you're trying to combine values of different types that are not compatible.
3.The error means that Python looks for something named x but finds nothing defined by that name.
# syntax error
# type error
# name error
